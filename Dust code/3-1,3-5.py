    ...
    while check_virtual_volume(rootdir, virtpath, limit_mb=500) is True:
        for user, df in train_data_dict.items():
            ...
            for group_id, group_df in df_proc.groupby('group'):
                ...
                # 各groupのtimestampの最初の値を0に調整
                group_df['timestamp'] = group_df['timestamp'] - group_df['timestamp'].iloc[0]
                
                # 補間用の新しいtimestampの生成（num_timestampsが指定されていれば均等割り、なければ元の値をそのまま）
...
if __name__ == '__main__':
    ...
    #                                round_values=False)
    #         f1 = HAR_evaluation(f"outlier_{outlier_method}_interp_{interp_method}_False",'/root/Virtual_Data_Generation/data/virtual')
    ...
    #                                interpolation_method=interp_method,
    #                                round_values=True)
    #         f1 = HAR_evaluation(f"outlier_{outlier_method}_interp_{interp_method}_True",'/root/Virtual_Data_Generation/data/virtual')
    #         print(f"Outlier method: {outlier_method}, Interpolation method: {interp_method} -> f1 score: {f1}")
    #         f1_scores.append(f1)
    # print(f1_scores)
    # print("All processing done.")
