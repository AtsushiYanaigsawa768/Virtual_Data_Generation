...
train_users = ['U0101','U0102', 'U0103', 'U0104', 'U0106', 'U0109',
...
if __name__ == '__main__':
    ...
    virtual_dir = '/root/Virtual_Data_Generation/data/virtual'
    plot_dir = '/root/Virtual_Data_Generation/plots'
    os.makedirs(plot_dir, exist_ok=True)
    os.makedirs(virtual_dir, exist_ok=True)
    
    # # # # List of interpolation methods to test
    interpolation_methods = [
        'linear', 'rbf_inverse', 'rbf_multiquadric', 'rbf_gaussian',
        'rbf_linear', 'rbf_cubic', 'fft_hann', 'fft_Welch', 'fft_Blackman-Harris', 'akima', 'pchip', 
    ]
    
    # # Dictionary to store generated dataframes
    generated_dfs = {}
    
    # # Generate data using each interpolation method
    # for interp_method in interpolation_methods:
    #     # Clear previous files
    #     delete_csv_files(virtual_dir)
        
    #     # Generate data with current interpolation method
    #     process_csv_files(
    #         output_dir=virtual_dir,
    #         num_timestamps=100,
    #         outlier_method='savgol',
    #         interpolation_method=interp_method,
    #         round_values=True,
    #         distribution_type='normal'
    #     )
        
    #     # Find the generated U0101 file
    #     for file in os.listdir(virtual_dir):
    #         if file.startswith('U0101_'):
    #             file_path = os.path.join(virtual_dir, file)
    #             # Read only the first 100 rows
    #             df = pd.read_csv(file_path).head(100)
    #             generated_dfs[interp_method] = df
    #             break
    
    # # # # 1. Load the real data
    real_file_path = '/root/Virtual_Data_Generation/data/real/U0101-S0100.csv'
    real_df = pd.read_csv(real_file_path, usecols=selected_columns)
    # # Get only the first 100 rows
    real_df = real_df.head(100)
    
    # # # Create a single figure with subplots in two columns
    num_methods = len(generated_dfs)
    nrows = (num_methods + 1) // 2  # Calculate number of rows needed
    
    fig, axes = plt.subplots(nrows, 2, figsize=(15, 4*nrows))
    # Flatten axes array for easier indexing
    axes = axes.flatten() if nrows > 1 else [axes]
    
    # Add each method as a subplot
    for i, (method, synth_df) in enumerate(generated_dfs.items()):
        ax = axes[i]
        
        # Plot real data in blue
        ax.plot(real_df.index[:len(real_df)], real_df['atr01/acc_x'][:len(real_df)], 
                color='blue', label='Original Data')
        
        # Plot synthetic data in red/orange
        ax.plot(synth_df.index[:len(synth_df)], synth_df['atr01/acc_x'][:len(synth_df)], 
                color='orangered', label=f'Interpolation')
        ...
        ax.set_title(f'({subplot_label}) {method}', fontsize=18)
        ax.set_xlabel('Sample Index')
        ax.set_ylabel('atr01/acc_x')
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(plot_dir, 'comparison_all_methods.png'), dpi=300, bbox_inches='tight')
    plt.savefig(os.path.join(plot_dir, 'comparison_all_methods.eps'), format='eps', dpi=300, bbox_inches='tight')
    plt.close()

    # Generate data using only rbf_inverse interpolation with savgol filtering
    # Clear previous files
    # delete_csv_files(virtual_dir)
    
    # # Generate data with specified combination
    # process_csv_files(
    #     output_dir=virtual_dir,
    #     num_timestamps=100,
    #     outlier_method='savgol',
    ...
    #         file_path = os.path.join(virtual_dir, file)
    #         # Read only the first 100 rows
    #         synth_df = pd.read_csv(file_path).head(100)
    #         break
    
    # # Create a single figure for the specific combination
    # plt.figure(figsize=(10, 6))
    
    # # Plot real data in blue
    # plt.plot(real_df.index[:len(real_df)], real_df['atr01/acc_x'][:len(real_df)], 
    #         color='blue', label='Original Data')
    
    # # Plot synthetic data in red/orange
    # if synth_df is not None:
    #     plt.plot(synth_df.index[:len(synth_df)], synth_df['atr01/acc_x'][:len(synth_df)], 
    #             color='orangered', label='Synthetic Data (rbf_inverse + savgol)')
    
    # # Set y-axis limits
    # plt.ylim([0, 1])
    
    # # Add other plot elements
    # plt.title('Interpolation: rbf_inverse, Filter: savgol', fontsize=18)
    # plt.xlabel('Sample Index')
    # plt.ylabel('atr01/acc_x')
    # plt.legend()
    # plt.grid(True, alpha=0.3)
    
    # # Save the figure
    # plt.savefig(os.path.join(plot_dir, 'rbf_inverse_savgol.png'), dpi=300, bbox_inches='tight')
    # plt.savefig(os.path.join(plot_dir, 'rbf_inverse_savgol.eps'), format='eps', dpi=300, bbox_inches='tight')
    # plt.close()
    
    # print("rbf_inverse with savgol filter plot generated.")
    # # # Still generate individual plots for closer inspection
    # # for method, synth_df in generated_dfs.items():
    # #     plt.figure(figsize=(10, 6))
    ...
    # #     # Plot synthetic data in red/orange
    # #     plt.plot(synth_df.index[:len(synth_df)], synth_df['atr01/acc_x'][:len(synth_df)], 
    # #             color='orangered', label=f'Interpolation: {method}')
    ...
    # #     # Save the figure
    # #     plt.savefig(os.path.join(plot_dir, f'comparison_{method}.png'), dpi=300, bbox_inches='tight')
    # #     plt.savefig(os.path.join(plot_dir, f'comparison_{method}.eps'), format='eps', dpi=300, bbox_inches='tight')
    # #     plt.close()
