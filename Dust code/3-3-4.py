...
if __name__ == '__main__':
    ...
    for outlier_method in ['none']:
        for interp_method in [ "interpolate","bspline",'fft_hann', 'fft_Welch', 'fft_Blackman-Harris','akima','pchip', 'rbf_inverse', 'rbf_multiquadric', 'rbf_gaussian', 'rbf_linear', 'rbf_cubic', ]:
            process_csv_files(output_dir='/root/Virtual_Data_Generation/data/virtual',
                                   num_timestamps=True,
                                   outlier_method=outlier_method,
                                   interpolation_method=interp_method,
                                   round_values=False)
            f1 = HAR_evaluation(f"random_outlier_{outlier_method}_interp_{interp_method}_False",'/root/Virtual_Data_Generation/data/virtual')
    ...
    f1_scores = []
    for outlier_method in ['none']:
        for interp_method in [ "interpolate","bspline",'rbf_inverse', 'rbf_multiquadric', 'rbf_gaussian', 'rbf_linear', 'rbf_cubic', 'fft_hann', 'fft_Welch', 'fft_Blackman-Harris','pchip', 'akima',]:
            process_csv_files(output_dir='/root/Virtual_Data_Generation/data/virtual',
                                   num_timestamps=True,
                                   outlier_method=outlier_method,
                                   interpolation_method=interp_method,
                                   round_values=True)
            f1 = HAR_evaluation(f"random_outlier_{outlier_method}_interp_{interp_method}_True",'/root/Virtual_Data_Generation/data/virtual')
            print(f"Outlier method: {outlier_method}, Interpolation method: {interp_method} -> f1 score: {f1}")
            f1_scores.append(f1)
    print(f1_scores)
    print("All processing done.")
