...
if __name__ == '__main__':
    ...
    # df_proc = df.copy()
    # # タイムスタンプを先頭0起点に調整
    # df_proc['timestamp'] = df_proc['timestamp'] - df_proc['timestamp'].iloc[0]
    
    # # 元データのタイムスタンプ
    # original_timestamps = df_proc['timestamp'].values
    
    # # 各補間手法に対して処理とグラフ描画を行う
    # interpolation_methods = [
    #     'linear', 'rbf_inverse','fft_hann', 'akima', 'pchip', 
    # ]
    ...
    # # Set up the figure with 1 column and more space between subplots
    # n_methods = len(interpolation_methods)
    # fig, axs = plt.subplots(n_methods, 1, figsize=(12, 4*n_methods))
    
    # # If only one method, ensure axs is still iterable
    # if n_methods == 1:
    #     axs = [axs]
    ...
    # # Prepare subplot labels (a, b, c, ...)
    # labels = [f"({chr(97+i)})" for i in range(n_methods)]  # 97 is ASCII for 'a'
    
    # for i, interp_method in enumerate(interpolation_methods):
    #     # New timestamps (same as original in this example)
    #     new_timestamps = np.linspace(original_timestamps.min(), original_timestamps.max(), len(original_timestamps))
        
    #     # Filter outliers using savgol filter
    #     filtered_series = apply_outlier_filter(df_proc['atr01/acc_x'], method='none')
        
    #     # Perform interpolation
    #     interp_values = perform_interpolation(
    #         original_timestamps,
    #         filtered_series.values,
    #         new_timestamps,
    ...
    #     ax = axs[i]
    #     ax.plot(original_timestamps, df_proc['atr01/acc_x'], 'b-', label='Original', linewidth=2)
    #     ax.plot(new_timestamps, interp_values, 'r-', label='Interpolated', linewidth=2)
        
    #     # Configure subplot with larger labels
    #     ax.set_ylim([0, 1])
    ...
    #     # Create display name by extracting the main method name
    #     if 'rbf_' in interp_method:
    #         display_name = 'rbf(inverse)' 
    #     elif 'fft_' in interp_method:
    #         display_name = 'fft(hann)'
    #     else:
    #         display_name = interp_method
        
    #     ax.set_title(f"{labels[i]} {display_name}", fontsize=30, pad=35)
    
    # # Adjust layout
    # plt.tight_layout()
    
    # # Save the figure as EPS
    # plot_file = os.path.join(plot_dir, 'original_interpolation_methods_comparison.eps')
    # plt.savefig(plot_file, format='eps', dpi=600, bbox_inches='tight')
    # plt.savefig(plot_file.replace('.eps', '.png'), format='png', dpi=600, bbox_inches='tight')
    # plt.close()
    ...
    # 各フィルター手法に対して処理とグラフ描画を行う
    filter_methods = [
        'none', 'moving_average', 'ransac', 'wavelet',  'savgol'
    ]
    ...
    # Set up the figure with 2 columns (pchip and rbf_inverse) and 6 rows (filter methods)
    n_methods = len(filter_methods)
    fig, axs = plt.subplots(n_methods, 2, figsize=(24, 4*n_methods))
    
    # Add more spacing between subplots
    plt.subplots_adjust(hspace=0.4, wspace=0.3)
    
    # Prepare subplot labels (a, b, c, ...)
    labels = [f"({chr(97+i)})" for i in range(n_methods)]
    
    for i, filter_method in enumerate(filter_methods):
        # New timestamps (same as original in this example)
        new_timestamps = np.linspace(original_timestamps.min(), original_timestamps.max(), len(original_timestamps))
        
        # Filter outliers using the specified filter method
        filtered_series = apply_outlier_filter(df_proc['atr01/acc_x'], method=filter_method)
        
        # PCHIP interpolation (left column)
        interp_values_pchip = perform_interpolation(
            original_timestamps,
            filtered_series.values,
            new_timestamps,
            method='pchip'
        ...
        # Plot PCHIP in the left column
        ax_left = axs[i, 0]
        ax_left.plot(original_timestamps, df_proc['atr01/acc_x'], 'b-', label='Original', linewidth=2)
        ax_left.plot(new_timestamps, interp_values_pchip, 'r-', label='Filtered & Interpolated', linewidth=2)
        
        # Configure left subplot
        ax_left.set_ylim([0, 1])
        ax_left.set_xlabel('Time', fontsize=16)
        ax_left.set_ylabel('Acceleration', fontsize=16)
        ax_left.legend(fontsize=14)
        ax_left.grid(True)
        ax_left.tick_params(axis='both', which='major', labelsize=14)
        
        # Create display name with appropriate prefix
        if filter_method == 'none':
            display_name = 'no filter'
        elif filter_method == 'moving_average':
            display_name = '(f) moving average'
        elif filter_method == 'ransac':
            display_name = '(g) ransac'
        elif filter_method == 'wavelet':
            display_name = '(h) wavelet'
        elif filter_method == 'savgol':
            display_name = '(i) savitzky-golay'
        else:
            display_name = filter_method
            
        ax_left.set_title(f"(d) pchip + {display_name}", fontsize=30, pad=35)
        
        # RBF_inverse interpolation (right column)
        interp_values_rbf = perform_interpolation(
            original_timestamps,
            filtered_series.values,
            new_timestamps,
            method='rbf_inverse'
        ...
        # Plot RBF_inverse in the right column
        ax_right = axs[i, 1]
        ax_right.plot(original_timestamps, df_proc['atr01/acc_x'], 'b-', label='Original', linewidth=2)
        ax_right.plot(new_timestamps, interp_values_rbf, 'r-', label='Filtered & Interpolated', linewidth=2)
        
        # Configure right subplot
        ax_right.set_ylim([0, 1])
        ax_right.set_xlabel('Time', fontsize=16)
        ax_right.set_ylabel('Acceleration', fontsize=16)
        ax_right.legend(fontsize=14)
        ax_right.grid(True)
        ax_right.tick_params(axis='both', which='major', labelsize=14)
        
        ax_right.set_title(f"(b) rbf(inverse) + {display_name}", fontsize=30, pad=35)
    # Adjust layout
    plt.tight_layout()
    
    # Save the combined figure as EPS
    plot_file = os.path.join(plot_dir, 'combined_filter_methods_comparison.eps')
    plt.savefig(plot_file, format='eps', dpi=600, bbox_inches='tight')
    plt.savefig(plot_file.replace('.eps', '.png'), format='png', dpi=600, bbox_inches='tight')
    plt.close()
    print(f"Saved combined comparison plot to {plot_file}")
    print("All plots generated.")
    # 元のコード（必要に応じて）
    # f1_scores = {}
    ...
    #                 f1 = HAR_evaluation(f"test_{dist_type}_{outlier_method}_{interp_method}_{run}")
    #                 print(f"F1 score for {dist_type} ({outlier_method}, {interp_method}) run {run}: {f1:.4f}")
