...
def perform_interpolation(x, y, new_x, method='pchip', **kwargs):
    ...
    # elif method == 'newton':
    #     # 使用する点が多すぎる場合は数値不安定性が発生するため、閾値を設ける
    #     if len(x) > 30:
    #         print("Warning: Too many points for Newton interpolation; falling back to PCHIP interpolation.")
    #         return interpolate_pchip(x, y, new_x)
    #     else:
    #         newton_poly = KroghInterpolator(x, y)
    #         return newton_poly(new_x)
    elif method == 'hybrid':
...
def dtw_align_paths(seq1, seq2, dist=lambda x, y: abs(x - y)):
    """
    Compute DTW alignment path between two sequences seq1 and seq2.
    Returns a list of index pairs (i, j) that represent the alignment.
    """
    n, m = len(seq1), len(seq2)
    # DTW distance matrix
    dtw = np.full((n+1, m+1), np.inf)
    dtw[0, 0] = 0
    # Pointers matrix (for path reconstruction)
    ptr = np.zeros((n+1, m+1, 2), dtype=int)
    
    for i in range(1, n+1):
        for j in range(1, m+1):
            cost = dist(seq1[i-1], seq2[j-1])
            # Choose min of three neighboring cells
            choices = [dtw[i-1, j], dtw[i, j-1], dtw[i-1, j-1]]
            idx = np.argmin(choices)
            dtw[i, j] = cost + choices[idx]
            # ptr: 0 -> came from (i-1, j), 1 -> from (i, j-1), 2 -> from (i-1, j-1)
            if idx == 0:
                ptr[i, j] = [i-1, j]
            elif idx == 1:
                ptr[i, j] = [i, j-1]
...
def process_csv_files(output_dir, num_timestamps=None,
    ...
    その後、train_data_dict内の各CSVファイルについて以下の処理を行う。
      1. 指定された列のみ抽出し、操作(operation)の変化毎にgroupを作成
      ...
      6. ユニークな名前でCSV出力
      7. 出力後、出力ディレクトリの容量がlimit_mb未満かをチェックし、条件を満たすまでループ処理を継続
    """
    # 出力先内のCSVをすべて削除
    delete_csv_files(output_dir)
    rootdir = '/root/Virtual_Data_Generation'
    virtpath = '/data/virtual'
    # ループ：出力先容量が500MB未満になるまで処理（容量が十分になる＝limitに達したら終了）
    while check_virtual_volume(rootdir, virtpath, limit_mb=500) is True:
        for user, df in train_data_dict.items():
            ...
            for group_id, group_df in df_proc.groupby('group'):
                ...
                # 各 group の timestamp を先頭の値でシフトして 0 に調整
                group_df['timestamp'] = group_df['timestamp'] - group_df['timestamp'].iloc[0]
                
                # 補間用の新しい timestamp の生成
                if num_timestamps is True:
                    stats_df = pd.read_csv('/root/operation_segment_stats_robust.csv')
                    op_val = group_df['operation'].iloc[0]
                    stats_row = stats_df[stats_df['operation'] == op_val]
                    stats_row = stats_df[stats_df['operation'] == op_val]
                    if not stats_row.empty:
                        median_val = stats_row['median'].iloc[0]
                        std_val = stats_row['std'].iloc[0]
                        current_num_timestamps = 0
                        while current_num_timestamps < median_val-std_val:
                            current_num_timestamps = int(np.random.normal(median_val, std_val))
                    else:
                        current_num_timestamps = 10
                ...
                # 新たな DataFrame を作成（補間後の行数は new_timestamps の長さになる）
                new_group_df = pd.DataFrame({'timestamp': new_timestamps})
                
                # 各説明変数に対して外れ値フィルタと補間処理を実施
                for col in explanatory_columns:
                    ...
                    interp_values = perform_interpolation(
                                        group_df['timestamp'].values,
                                        filtered_series.values,
                                        new_timestamps,
                                        method=interpolation_method
                                    )
                    # new_group_df に補間結果を追加
                    new_group_df[col] = interp_values

                # 必要に応じて、その他の定数情報（たとえば operation など）を追加
            ...
            # round_valuesがTrueの場合、数値を少数第5位で四捨五入
            if round_values:
            ...
            # print(f"Saved processed file for {user} to {output_file}")
...
if __name__ == '__main__':
    ...
    f1_scores = []
    for trial in range(1, 31):  # 30 times
        # Set round_values to True for even trials, False for odd trials
        round_val = (trial % 2 == 0)
        
        # Use trial number in the HAR_evaluation function call
        outlier_method = 'savitzky_golay'
        interp_method = 'rbf_inverse'
        
        process_csv_files(output_dir='/root/Virtual_Data_Generation/data/virtual',
                          num_timestamps=True,
                          outlier_method=outlier_method,
                          interpolation_method=interp_method,
                          round_values=round_val)
        
        f1 = HAR_evaluation(f"Trial_{trial}_outlier_{outlier_method}_interp_{interp_method}_round_{round_val}",
                           '/root/Virtual_Data_Generation/data/virtual')
        
        print(f"Trial {trial}: Outlier method: {outlier_method}, Interpolation method: {interp_method}, "
              f"Round values: {round_val} -> f1 score: {f1}")
        f1_scores.append(f1)
