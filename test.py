...
def curve_fit_implementation(random_times, df_grouped):
    ...
    def func(x, a, b, c, d, e, f, g, h, i, j, k, l):
        return (a * x**9 + b * x**8 + c * x**7 + d * x**6 + e * x**5 + 
                f * x**4 + g * x**3 + h * x**2 + i * x + j +
                k * np.cos(x) + l * np.sin(x))
...
def main_loop(operation_int,repeat_int,volume_limit=500,method='linear'):
    output_csv = rootdir +  r'/data/test1' + f'/interpolated_{operation_int}.csv'
    # 既存の出力ファイルがあれば削除しておく
    ...
    while volume_checker:
        ...
        volume_checker = check_virtual_volume(rootdir,r'/data/test1',volume_limit)

if __name__ == "__main__":
    i = 0
    for operation_int in [100,200,300,400,500,600,700,800,900,1000,8100]:
        i += 1
        main_loop(operation_int=operation_int,repeat_int=600,volume_limit=i * 10,method='linear')