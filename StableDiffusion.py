...
# Load label statistics from the train summary CSV file in the Statistics directory
summary_csv_path = os.path.join(rootdir, "Statistics", "train_data_summary_by_op.csv")
summary_df = pd.read_csv(summary_csv_path)

label_statistic_mean = {}
label_statistic_std = {}
# Load duration_label from the train data summary CSV file
...
try:
    ...
    print(f"Error: The file {summary_csv_path} does not exist.")
    summary_df = pd.DataFrame()
except pd.errors.EmptyDataError:
    print(f"Error: The file {summary_csv_path} is empty.")
    summary_df = pd.DataFrame()
except pd.errors.ParserError:
    print(f"Error: The file {summary_csv_path} could not be parsed.")
    summary_df = pd.DataFrame()
label_statistic_mean = {}
label_statistic_var = {}
...
required_columns = [
    'operation',
    'atr01/acc_x_block_mean', 'atr01/acc_y_block_mean', 'atr01/acc_z_block_mean',
    'atr02/acc_x_block_mean', 'atr02/acc_y_block_mean', 'atr02/acc_z_block_mean',
    'atr01/acc_x_block_var', 'atr01/acc_y_block_var', 'atr01/acc_z_block_var',
    'atr02/acc_x_block_var', 'atr02/acc_y_block_var', 'atr02/acc_z_block_var'
]

if all(col in summary_df.columns for col in required_columns):
    for _, row in summary_df.iterrows():
        op = row['operation']
        label_statistic_mean[op] = [
            row['atr01/acc_x_block_mean'],
            row['atr01/acc_y_block_mean'],
            row['atr01/acc_z_block_mean'],
            row['atr02/acc_x_block_mean'],
            row['atr02/acc_y_block_mean'],
            row['atr02/acc_z_block_mean']
        ]

        label_statistic_var[op] = [
            row['atr01/acc_x_block_var'],
            row['atr01/acc_y_block_var'],
            row['atr01/acc_z_block_var'],
            row['atr02/acc_x_block_var'],
            row['atr02/acc_y_block_var'],
            row['atr02/acc_z_block_var']
        ]
for _, row in summary_df.iterrows():
    op = row['operation']
    label_statistic_mean[op] = {
        'block_size_mean': row['block_size_mean'],
        'atr01/acc_x_block_mean': row['atr01/acc_x_block_mean'],
        'atr01/acc_y_block_mean': row['atr01/acc_y_block_mean'],
        'atr01/acc_z_block_mean': row['atr01/acc_z_block_mean'],
        'atr02/acc_x_block_mean': row['atr02/acc_x_block_mean'],
        'atr02/acc_y_block_mean': row['atr02/acc_y_block_mean'],
        'atr02/acc_z_block_mean': row['atr02/acc_z_block_mean']
    }
    label_statistic_std[op] = {
        'block_size_var': row['block_size_var'],
        'atr01/acc_x_block_var': row['atr01/acc_x_block_var'],
        'atr01/acc_y_block_var': row['atr01/acc_y_block_var'],
        'atr01/acc_z_block_var': row['atr01/acc_z_block_var'],
        'atr02/acc_x_block_var': row['atr02/acc_x_block_var'],
        'atr02/acc_y_block_var': row['atr02/acc_y_block_var'],
        'atr02/acc_z_block_var': row['atr02/acc_z_block_var']
    }
...
def create_real_dataset():
    ...
    # Iterate over each operation and normalize accordingly
    for op in all_label_list:
        # Get indices for the current operation
        op_indices = all_data[all_data['operation'] == op].index
        # Get mean and std for the current operation
        op_mean = np.array(list(label_statistic_mean[op].values())[:6])
        op_std = np.array(list(label_statistic_std[op].values())[:6])
        # Normalize the data for the current operation
        normalized_data[op_indices] = (sensor_data[op_indices] - op_mean) / op_std

    # Replace sensor_data with normalized data
...
while memory_checker:
    ...
    for label_value in all_label_list:
        ...
        # 逆正則化: 正規化前のスケールに戻す
        op_mean = np.array(list(label_statistic_mean[label_value].values())[1:])
        op_std = np.array(list(label_statistic_std[label_value].values())[1:])
        generated_x = generated_x * torch.from_numpy(op_std).float().to(device) + torch.from_numpy(op_mean).float().to(device)
        ...
        # CSVファイルとして保存（例：generated_8100.0.csv）
